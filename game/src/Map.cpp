...
entityCoordinates Map::setCoordinates(const utils::Coordinate& coord) { return std::make_pair(coord.x, coord.y); }
...
void Map::loadEntity(const utils::Coordinate& coord, const Entity& entity) {
    if (entities_.count(setCoordinates(coord))) {
        // SPDLOG_ERROR(("At x="+coordinate.x+" and y="+coordinate.y+" there is already an entity on map "+id_+".").c_str());
        // throw std::invalid_argument(("At x="+coordinate.x+" and y="+coordinate.y+" there is already an entity on map "+id_+".").c_str());
        return;
    }
    entityCoordinates ent_coord = std::make_pair(coord.x, coord.y);
    entities_.insert(std::make_pair(ent_coord, entity));
}

void Map::replaceEntity(const utils::Coordinate& coordinate, const Entity& entity) { entities_.at(setCoordinates(coordinate)) = entity; }

void Map::loadNextEntity(const NPC& entity) {
...
void Map::loadEntities(const std::vector<utils::Coordinate>& coord, const std::vector<Entity>& entities) {
