...
Player::Player(uint32_t id, uint8_t type, std::string name, _movement::Movement movement, InteractionEntity interaction, uint8_t status) : Entity(id, type, name, movement, interaction) {
    this->status_ = status;
}
